...
Claude Code natively renders a status line from a shell command you configure in `settings.json`. **cc-cream *is* that command** — a small Node runtime that reads the JSON Claude Code pipes to it on stdin and prints a formatted, colored, up-to-two-row bar — **plus a consent-based installer** that writes the one `statusLine` line into the user's settings.json. The model never sees the output, so it costs zero tokens.
...
| Context | `ctx:19% (38k)` | `context_window.used_percentage`; compact magnitude = `total_input_tokens` (input-only, to match `used_percentage`) | current-context snapshot | `<30 green · 30–40 amber · 40–50 orange · ≥50 red` of the **fullness reference** chosen by `ctx.basis` — `window` (default) or `ceiling` (§4.4; research earmark §11 resolved — thresholds are now evidence-based) |
...
- **Context** — colors against a configurable **fullness reference** (`ctx.basis`), so the same `<30 / 30–40 / 40–50 / ≥50` band shape can mean two things:
  - **`basis: "window"` (default).** Bands are percent of the *real* context window (`used_percentage`); red begins at 50% — the research-grounded hard ceiling (Veseli et al. 2025: positional-bias qualitatively shifts above ~50%; Chroma Context Rot 2025: degradation underway well before the limit). Amber (30–40%) = prune zone; orange (40–50%) = compact-now action zone; red (≥50%) = start fresh, no safe compaction window remains. Honest about the window, but on a large window the early warning is weak: on a 1M model the ~100–120k degradation zone is only ~12% → solid green. Use `basis: "ceiling"` to restore early-warning on large windows. No-regression default (the v1 shipped behavior).
  ...
  Thresholds are evidence-based (Veseli et al. 2025, Chroma Context Rot 2025, Du et al. 2025): 50% is the Veseli positional-bias inflection point and the research consensus ceiling; the three zones below it map to prune → compact → start-fresh actions. The ceiling basis closes the §11 earmark by making the practical ceiling a first-class, model-independent knob.
...
**Research earmark**
- **Context-fullness thresholds — Resolved (2026-05-27).** Updated to `<30 green / 30–40 amber / 40–50 orange / ≥50 red` (evidence: Veseli et al. 2025 — 50% is the positional-bias inflection point; Chroma Context Rot 2025 — continuous degradation across all 18 tested models; Du et al. 2025 — reasoning degrades with input length even with perfect retrieval). The `ctx.basis: "ceiling"` option closes the large-window blind spot (§14.5). Earmark fully resolved.
...
- Runs as Node JavaScript on macOS/Windows/Linux with no external dependencies; installs via marketplace, npm, or raw GitHub by writing one consented line into settings.json.
...
1. **Distribution scope — raw JavaScript on GitHub is the only v1 channel.** npm `bin` packaging and the marketplace plugin wrapper move to the v2+ backlog (§11). The no-runtime-dependency engine constraint (§7) is unchanged — it's precisely what makes those deferred channels cheap to add later. The consent-based installer (§7) still ships in v1; it simply targets the raw JavaScript path.
