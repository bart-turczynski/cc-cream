Feature: TTL segment — cache-warmth countdown
  ...
  I want ttl:MM showing time remaining in minutes before the cache expires
  So that I get an early warning before the cache goes cold

  # PRD §4.1, §4.5, §8, §9. ttl = (resolved TTL) − (now − mtime(transcript_path)),
  # floored at 0. Count-down in whole minutes (refresh is 60s).
  ...
  Scenario: Hidden, not ttl:0, when transcript_path is absent or cannot be stat-ed
